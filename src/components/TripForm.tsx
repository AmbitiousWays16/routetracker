...
const tripFormSchema = z.object({
  ...
  fromAddress: z.string().trim().min(1, 'From address is required').max(500, 'Address is too long (max 500 characters)'),
  toAddress: z.string().trim().min(1, 'To address is required').max(500, 'Address is too long (max 500 characters)'),
  businessPurpose: z.string().trim().min(1, 'Business purpose is required').max(500, 'Business purpose is too long (max 500 characters)'),
...
export const TripForm = ({ onSubmit, onCalculateRoute, programs, programsLoading, isAdmin, onAddProgram, onUpdateProgram, onDeleteProgram }: TripFormProps) => {
  ...
  const handleProgramChange = (programName: string) => {
    ...
    if (selectedProgram?.address) setToAddress(selectedProgram.address);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const validation = tripFormSchema.safeParse({ date, fromAddress, toAddress, businessPurpose, program });
    if (!validation.success) { toast.error(validation.error.errors[0].message); return; }
    if (miles <= 0) { toast.error('Please calculate the route first'); return; }

    onSubmit({ date: validation.data.date, fromAddress: validation.data.fromAddress, toAddress: validation.data.toAddress, businessPurpose: validation.data.businessPurpose, program: validation.data.program, miles, routeUrl, routeMapData: routeMapData || undefined });

    if (isRoundTrip) {
      onSubmit({ date: validation.data.date, fromAddress: validation.data.toAddress, toAddress: validation.data.fromAddress, businessPurpose: `${validation.data.businessPurpose} (Return)`, program: validation.data.program, miles, routeUrl, routeMapData: routeMapData || undefined });
    }

    setFromAddress(''); setToAddress(''); setBusinessPurpose(''); setProgram(''); setMiles(0); setRouteUrl(''); setRouteMapData(null); setIsRoundTrip(false);
  };
    ...
    <Card className="shadow-card animate-fade-in">
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center justify-between text-lg font-semibold">
          <span className="flex items-center gap-2"><Car className="h-5 w-5 text-primary" />Add New Trip</span>
          {isAdmin && <ProgramManager programs={programs} loading={programsLoading} onAdd={onAddProgram} onUpdate={onUpdateProgram} onDelete={onDeleteProgram} />}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="program" className="flex items-center gap-1.5 text-sm font-medium"><FileText className="h-3.5 w-3.5 text-muted-foreground" />Program</Label>
              <Select value={program} onValueChange={handleProgramChange}>
                <SelectTrigger className="h-10"><SelectValue placeholder={programsLoading ? 'Loading...' : 'Select program'} /></SelectTrigger>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="from" className="flex items-center gap-1.5 text-sm font-medium"><MapPin className="h-3.5 w-3.5 text-success" />From Address</Label>
              <AddressAutocomplete id="from" placeholder="Enter starting address" value={fromAddress} onChange={setFromAddress} programs={programs} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="to" className="flex items-center gap-1.5 text-sm font-medium"><MapPin className="h-3.5 w-3.5 text-destructive" />To Address</Label>
              <Input id="to" placeholder="Enter destination address" value={toAddress} onChange={(e) => setToAddress(e.target.value)} className="h-10" />
            </div>
          </div>
